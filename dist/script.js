...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])(intervalId, modalState);
...
var changeModalState = function changeModalState(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#width'); // Отдельная функция валидации тоже отдельный мдуль 
    ...
    elem.forEach(function (item, i) {
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          // Ноднейм дает имя ноды
          case 'SPAN':
            // Ноднейм всегда отдает в большом регистре
            state[prop] = i;
            console.log(state);
            break;
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var formData = new FormData(item); // соберет объект из данных формы

      if (item.getAttribute('data-calc') === 'end') {
        // Добавили все данные из свитча, в котором мы собирали данные о заказе пользователя
        for (var key in state) {
          // Перебираем объект в JS
...
var modals = function modals(modalId, state) {
  console.log(modalId);

  function bindModal(triggerSelector, modalSelector, closeSelector) {
    var closeClickIverlay = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    ...
    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      // Проверяем в таб ли кликнул пользак 
      tab.forEach(function (item, i) {
        if (target == item || target.parentNode == item) {
          // Если пользователь пользователь кликнул на таб, который перебирается (или на его потомка например ссылку или изображение)
          // console.log(target)
